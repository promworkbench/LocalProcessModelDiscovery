...
public abstract class AbstractAllOptAlignmentsGraphAlg<D extends AbstractPDelegate<T>, T extends Tail> extends AbstractAllOptAlignmentsAlg<D,T>{
	protected int threads = Runtime.getRuntime().availableProcessors()-1;
	...
	protected int addReplayResults(D delegate, XTrace trace, MatchInstancesGraphRes r, TIntIntMap doneMap, XLog log, List<AllSyncReplayResult> col, int traceIndex, Map<Integer, AllSyncReplayResult> mapRes, final List<Object> suffixNodeInstance, final List<StepTypes> suffixStepTypes) {
		AllSyncReplayResult repResult = recordToResult(delegate, trace, r.filteredTrace, r.records, traceIndex, r.states, r.queuedStates, r.reliable, r.milliseconds, suffixNodeInstance, suffixStepTypes, new AllSyncReplayResult(), r.mapRecordToSameSuffix);
		if (mapRes == null) {
