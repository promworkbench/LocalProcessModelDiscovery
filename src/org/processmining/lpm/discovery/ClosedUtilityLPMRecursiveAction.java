...
public class ClosedUtilityLPMRecursiveAction extends RecursiveAction{
	...
	public ClosedUtilityLPMRecursiveAction(PluginContext context, UtilityLocalProcessModelParameters params, Set<String> lpmTreeSet, ProcessTree pt, int recursionDepth, Set<Character> transitionNamesForNonDuplicateMode, Set<Character> transitions, AtomicInteger petriNetsExplored, UtilityAlignmentScoredAcceptingPetrinetContainer topSet, Map<String, Integer> logActivityCountsMap, UtilityLocalProcessModel previousLPM){
	...
	protected void compute() {
		List<ClosedUtilityLPMRecursiveAction> subtasks = new ArrayList<ClosedUtilityLPMRecursiveAction>();
		...
		if(ptNew.getNodes().size()>0){
			...
			if(pwm!=null){
				...
				if(!frequencyMinimumMet || !avgEnabledTransMinimumMet || !languageRatioMinimumMet || !confidenceMinimumMet || !utilityMinimumMet){
					...
					if(previousLPM!=null && thisLPM.getUtility()>=previousLPM.getUtility())
			...
			for(Character other : transitions){
				...
				if(params.isDuplicateTransitions())
					subtasks.add(new ClosedUtilityLPMRecursiveAction(context, params, lpmTreeSet, newpt, recursionDepth+1, null, transitions, petriNetsExplored, topSet, logActivityCountsMap, previousLPM));
				else{
					Set<Character> newTransitionNames = new HashSet<Character>(transitions);
					newTransitionNames.remove(other);
					subtasks.add(new ClosedUtilityLPMRecursiveAction(context, params, lpmTreeSet, newpt, recursionDepth+1, newTransitionNames, transitions, petriNetsExplored, topSet, logActivityCountsMap, previousLPM));
				}
			}
			invokeAll(subtasks);
			return;
		...
		if(recursionDepth>=params.getNumTransitions() || (!params.isDuplicateTransitions() && transitionNamesForNonDuplicateMode.isEmpty()))
			...
			for(Manual manual : highestDepthNodes){
				...
				if(params.isDuplicateTransitions())
					...
					subtasks.add(new ClosedUtilityLPMRecursiveAction(context, params, lpmTreeSet, newPt, recursionDepth+1, transitionNamesForNonDuplicateMode, transitions, petriNetsExplored, topSet, logActivityCountsMap, previousLPM));
			}
			invokeAll(subtasks);
		}
	}
