...
@Plugin(
		name = "Detect redundancy in pragmatic LPM search", 
		parameterLabels = {}, 
	    returnLabels = {"Local Process Model Ranking"}, 
	    returnTypes = { ProcessTreeArray.class }
		)
public class PragmaticSearchRedundancyDetector{
	...
	@PluginVariant(variantLabel = "Detect redundancy in pragmatic LPM search", requiredParameterLabels = {})
	...
	private void recurseProcessTrees(PluginContext context, ProcessTree pt, int recursionDepth, Set<Character> transitionNamesForNonDuplicateMode){
		String treeAsString = pt.toString();
		...
		if(pt.getNodes().size()>0){
			PetrinetWithMarkings pwm = null;
			try {
				pwm = ProcessTree2Petrinet.convert(pt); // TODO: switch to new conversion plugin in PTConversion package, because old method is now deprecated
		...
		if(recursionDepth>=params.getNumTransitions() || (!params.isDuplicateTransitions() && transitionNamesForNonDuplicateMode.isEmpty()))
			...
			for(Manual manual : highestDepthNodes){
				...
				boolean isRoot = pt.getRoot().equals(manual);
