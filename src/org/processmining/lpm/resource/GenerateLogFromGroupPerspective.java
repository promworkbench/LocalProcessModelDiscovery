...
@Plugin(
		name = "Generate log from org:group perspective", 
		parameterLabels = {"Input Log"}, 
	    returnLabels = {"Resource Log"}, 
	    returnTypes = { XLog.class }
		)
public class GenerateLogFromGroupPerspective {
	@UITopiaVariant(affiliation = UITopiaVariant.EHV, author = "N. Tax", email = "n.tax@tue.nl")
	@PluginVariant(variantLabel = "Generate log from org:group perspective", requiredParameterLabels = {0})
	public XLog aggregateTracesByPeriodDefault(PluginContext context, XLog log) {
		return generateLogFromAttributePerspective(log, "org:group");
	}	
}